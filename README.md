...
Готовый jar: `build/libs/nhack-<version>.jar` (сейчас `nhack-1.0.9.jar`)
...
- `OreRadius` (дефолт 4) + `OreBlacklist` — если в радиусе от алмаза есть руда из блэклиста, метка не ставится:
  обфускатор сеет микс руд вперемешку, а настоящие жилы разных руд вплотную почти не срастаются. В ClickGUI
  блэклист открывается кнопкой в список из 8 руд с чекбоксами (крестик = руда в блэклисте), по умолчанию включены
  все: уголь, глубинный уголь, железо, глубинное железо, медь, глубинная медь, золото, глубинное золото.
...
Настройки: `Radius`, `MaxY`, `MaxPerChunk`, `AirMin`, `OreRadius`, `OreBlacklist`, `DiamondColor`.
