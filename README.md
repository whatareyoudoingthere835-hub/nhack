...
Готовый jar: `build/libs/nhack-<version>.jar` (сейчас `nhack-1.0.8.jar`)
...
`AntiXray` (дефолт `Auto`) — учёт анти-xray Paper engine mode 2: такой сервер подмешивает в данные чанка фейковую
руду по всем открытым поверхностям, а настоящую держит камнем и раскрывает block-update'ом, только когда рядом
добыли блок. В `Auto` модуль сам видит обфускацию (12+ открытых алмазов в чанке — ваниль так не генерирует), пишет
об этом в чат и переключается на честный режим: метки ставятся лишь по пакетам раскрытия
(`ClientboundBlockUpdatePacket` / `ClientboundSectionBlocksUpdatePacket`), а переход руда→камень снимает метку
фейка. `On` — честный режим сразу, `Off` — доверять руде из чанков (серверы без анти-xray). Настройки: `Radius`,
`MaxY`, `MaxPerChunk`, `AntiXray`, `DiamondColor`.
